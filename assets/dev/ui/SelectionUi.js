...
QuestUi.openSelectionUi = function (title, selection) {
    ...
    let ui = QuestUiTools.createUi({
        ...
        drawing: [
            ...
            {type: 'text', text: '', x: 500, y: 75, font: {color: $Color.BLACK, size: 60, align: 1}}
        ],
        elements: {
            close: {type: 'closeButton', x: 900, y: 40, bitmap: 'X', bitmap2: 'XPress', scale: 60/19}
        }
    }, {
        onClose () {
            ui = null
    ...
    selection.forEach(function (obj, index) {
        ...
        content.elements[index + '_btn'] = {
            type: 'image', x: 0, y: height, z: 1, bitmap: 'clear', width: 1000, height: 200,
            clicker: {
                onClick () {
        ...
        content.elements[index + '_text'] = {
            type: 'text', x: 500, y: height + 40, z: 2, text: obj.text,
            font: obj.darken
                ? { color: $Color.GRAY, size: 60, align: 1, cursive: true }
                : { color: $Color.BLACK, size: 60, align: 1, bold: true }
        }
